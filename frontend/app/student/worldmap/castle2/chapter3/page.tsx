...
const CHAPTER_KEY = 'castle2-chapter3';

export default function Castle2Chapter3Page() {
    ...
    const fetchQuizScore = async () => {
      if (currentScene === 'reward' && quiz?.id) {
        try {
          ...
          console.log('[Castle2-CHAPTER3] Fetched quiz attempts:', attempts);
          ...
          if (attempts && attempts.length > 0) {
            ...
            console.log('[Castle2-CHAPTER3] Most recent quiz score:', mostRecentScore);
  ...
  // Track lesson progress - simpler version for chapter3 (no dialogue task mappings)
  React.useEffect(() => {
  ...
  function handleDialogueComplete() {
    if (currentScene === 'opening') {
      ...
      playNarration('castle2-CHAPTER3-lesson-intro');
    ...
    if (userAnswer === question.correctAnswer) {
      ...
      setTimeout(() => {
        ...
        if (quizNumber === 1) {
          ...
          // Submit quiz attempt
          try {
            console.log('[Castle2-CHAPTER3] Submitting quiz with answers:', quizAnswers);
            console.log('[Castle2-CHAPTER3] Quiz config:', quiz.quiz_config);
    ...
    <div className={`${baseStyles.chapterContainer} ${baseStyles.castle2Theme}`}>
      ...
      {showProgressModal && (
        <ChapterProgressModal
          chapterTitle="Chapter 1: The Angle Forge"
        ...
        <div className={baseStyles.gameArea}>
          ...
          {currentScene === 'opening' && (
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              <h2 style={{ color: '#FFFD8F', fontSize: '2rem' }}>Welcome to The Angle Forge!</h2>
          ...
          {currentScene === 'lesson' && (
            <div style={{ textAlign: 'center', padding: '3rem' }}>
              <h2 style={{ color: '#FFFD8F', fontSize: '2rem', marginBottom: '2rem' }}>Complementary and Supplementary Angles</h2>
              <div style={{ fontSize: '1.2rem', color: '#B0CE88', lineHeight: '1.8', maxWidth: '800px', margin: '0 auto' }}>
                <p>💚 <strong>Complementary angles</strong> add up to 90°</p>
                <p>💙 <strong>Supplementary angles</strong> add up to 180°</p>
                <p>📐 Use these relationships to find missing angles!</p>
              </div>
            </div>
          )}
          ...
          {currentScene === 'minigame' && (
            <ComplementarySupplementaryMinigame
              question={{
                id: `minigame-${currentMinigameLevel}`,
                instruction: CHAPTER3_MINIGAME_LEVELS[currentMinigameLevel].description,
                correctAnswer: CHAPTER3_MINIGAME_LEVELS[currentMinigameLevel].correctAnswer.toString(),
                givenAngle: CHAPTER3_MINIGAME_LEVELS[currentMinigameLevel].givenAngle,
                relationship: CHAPTER3_MINIGAME_LEVELS[currentMinigameLevel].relationship,
                targetSum: CHAPTER3_MINIGAME_LEVELS[currentMinigameLevel].targetSum,
