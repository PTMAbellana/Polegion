...
export default function Chapter2Page() {
  ...
  const [completedTasks, setCompletedTasks] = useState({
    learnParallel: false,
    learnIntersecting: false,
    learnPerpendicular: false,
    learnSkew: false,
    ...
    const loadChapterData = async () => {
      if (!authLoading && userProfile?.id) {
        try {
          ...
          console.log('[Chapter2] Loading data for castle:', CASTLE_ID)
          const chaptersRes = await getChaptersByCastle(CASTLE_ID)
          console.log('[Chapter2] Chapters response:', chaptersRes)
          
          const chapter2 = chaptersRes.data?.find((ch: any) => ch.chapter_number === CHAPTER_NUMBER)
          
          if (!chapter2) {
            throw new Error('Chapter 2 not found')
          }
          
          console.log('[Chapter2] Found chapter 2:', chapter2)
          setChapterId(chapter2.id)
          
          const [quizzesRes, minigamesRes] = await Promise.all([
            getChapterQuizzesByChapter(chapter2.id),
            getMinigamesByChapter(chapter2.id)
          ])
          
          console.log('[Chapter2] Quizzes response:', quizzesRes)
          console.log('[Chapter2] Minigames response:', minigamesRes)
          
          if (quizzesRes.data && quizzesRes.data.length > 0) {
            console.log('[Chapter2] Setting quiz:', quizzesRes.data[0])
            setQuiz(quizzesRes.data[0])
          } else {
            console.warn('[Chapter2] No quizzes found!')
          }
          
          if (minigamesRes.data && minigamesRes.data.length > 0) {
            console.log('[Chapter2] Setting minigame:', minigamesRes.data[0])
            setMinigame(minigamesRes.data[0])
          } else {
            console.warn('[Chapter2] No minigames found!')
          }
          
        } catch (error) {
          console.error('[Chapter2] Failed to load chapter data:', error)
        } finally {
          setLoading(false)
        }
      }
  ...
  }, [completedTasks.learnParallel, completedTasks.learnIntersecting, completedTasks.learnPerpendicular, completedTasks.learnSkew])
  ...
  const handleNextMessage = async () => {
    if (currentScene === 'opening') {
      if (messageIndex < openingDialogue.length - 1) {
        ...
        setMessageIndex(0)
        setCompletedTasks(prev => ({ ...prev, learnParallel: true }))
      }
    } else if (currentScene === 'lesson') {
      if (messageIndex < lessonDialogue.length - 1) {
        ...
        if (messageIndex === 1) setCompletedTasks(prev => ({ ...prev, learnIntersecting: true }))
        if (messageIndex === 3) setCompletedTasks(prev => ({ ...prev, learnPerpendicular: true }))
        if (messageIndex === 4) setCompletedTasks(prev => ({ ...prev, learnSkew: true }))
      } else {
        if (chapterId) {
          try {
  ...
  const handleLineClick = (lineId: string) => {
    ...
    if (selectedPoints.includes(lineId)) {
      setSelectedPoints(prev => prev.filter(id => id !== lineId))
    } else {
      setSelectedPoints(prev => [...prev, lineId])
    }
  }

  const handleMouseMove = (e: any) => {
    // Not needed for line clicking in Chapter 2
  }

  const checkAnswer = async (lines: string[]) => {
    ...
    if (isCorrect) {
      ...
      if (question.type === 'Parallel Lines') {
        feedbackMessage += "These lines never meet and stay the same distance apart!"
      } else if (question.type === 'Perpendicular Lines') {
        feedbackMessage += "These lines form a perfect 90° right angle!"
      } else if (question.type === 'Intersecting Lines') {
        feedbackMessage += "These lines cross at an angle that's not 90°!"
      }
  ...
  const getScaledLines = () => {
    ...
    return question.lines.map((line: any) => ({
      ...line,
      x1: line.x1 * scaleX,
      y1: line.y1 * scaleY,
      x2: line.x2 * scaleX,
  ...
  const renderConnection = () => {
    // For Chapter 2, we don't need to render connections between selected items
    // Users select lines directly instead
    return null
  }
    ...
    <div className={styles.chapterContainer}>
      ...
      <div className={styles.topBar}>
        <div className={styles.chapterInfo}>
          ...
          <div>
            <h1 className={styles.chapterTitle}>Chapter 2: Paths of Power</h1>
      ...
      <div className={styles.mainContent}>
        <div className={styles.taskPanel}>
          ...
          <div className={styles.taskList} ref={taskListRef}>
            {[
              { key: 'learnParallel', label: 'Learn: Parallel Lines' },
              { key: 'learnIntersecting', label: 'Learn: Intersecting Lines' },
              { key: 'learnPerpendicular', label: 'Learn: Perpendicular Lines' },
              { key: 'learnSkew', label: 'Learn: Skew Lines' },
          ...
          {currentScene === 'lesson' && (
            <div className={styles.lessonContent}>
              <div className={styles.conceptGrid}>
                <div className={`${styles.conceptCard} ${completedTasks.learnParallel ? styles.revealed : styles.hidden}`}>
                  <h3>Parallel Lines</h3>
                  <div className={styles.visualDemo}>
                    <svg width="100%" height="80" viewBox="0 0 200 80">
                      <line x1="20" y1="25" x2="180" y2="25" stroke="#FFD700" strokeWidth="3" />
                      <line x1="20" y1="55" x2="180" y2="55" stroke="#FFD700" strokeWidth="3" />
                      <text x="10" y="30" fill="#FFD700" fontSize="12">L1</text>
                      <text x="10" y="60" fill="#FFD700" fontSize="12">L2</text>
                    </svg>
                  </div>
                  <p>Lines that never meet, no matter how far extended.</p>
                </div>
                
                <div className={`${styles.conceptCard} ${completedTasks.learnIntersecting ? styles.revealed : styles.hidden}`}>
                  <h3>Intersecting Lines</h3>
                  <div className={styles.visualDemo}>
                    <svg width="100%" height="80" viewBox="0 0 200 80">
                      <line x1="40" y1="20" x2="160" y2="60" stroke="#66BBFF" strokeWidth="3" />
                      <line x1="40" y1="60" x2="160" y2="20" stroke="#66BBFF" strokeWidth="3" />
                      <circle cx="100" cy="40" r="4" fill="#FF6B6B" />
                    </svg>
                  </div>
                  <p>Lines that cross at exactly one point.</p>
                </div>
                
                <div className={`${styles.conceptCard} ${completedTasks.learnPerpendicular ? styles.revealed : styles.hidden}`}>
                  <h3>Perpendicular Lines</h3>
                  <div className={styles.visualDemo}>
                    <svg width="100%" height="80" viewBox="0 0 200 80">
                      <line x1="100" y1="10" x2="100" y2="70" stroke="#FF6B6B" strokeWidth="3" />
                      <line x1="40" y1="40" x2="160" y2="40" stroke="#FF6B6B" strokeWidth="3" />
                      {/* Right angle marker */}
                      <rect x="95" y="35" width="10" height="10" fill="none" stroke="#FFD700" strokeWidth="2" />
                    </svg>
                  </div>
                  <p>Lines that intersect at a 90° angle (right angle).</p>
                </div>
                
                <div className={`${styles.conceptCard} ${completedTasks.learnSkew ? styles.revealed : styles.hidden}`}>
                  <h3>Skew Lines</h3>
                  <div className={styles.visualDemo}>
                    <svg width="100%" height="80" viewBox="0 0 200 80">
                      {/* Perspective representation of 3D skew lines */}
                      <line x1="30" y1="30" x2="90" y2="30" stroke="#4ECDC4" strokeWidth="3" opacity="0.7" />
                      <line x1="110" y1="50" x2="170" y2="20" stroke="#4ECDC4" strokeWidth="3" />
                      <text x="100" y="70" textAnchor="middle" fill="#AAA" fontSize="11">Not parallel, don't intersect</text>
                    </svg>
                  </div>
                  <p>Lines in 3D space that don't intersect and aren't parallel.</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className={styles.minigameContent}>
              <div className={styles.minigameCard}>
                ...
                <div className={styles.canvasContainer} ref={canvasContainerRef}>
                  ...
                  >
                    <Layer>
                      {getScaledLines().map((line: any) => {
                        const isSelected = selectedPoints.includes(line.id)
                        return (
                          <React.Fragment key={line.id}>
                            <Line
                              points={[line.x1, line.y1, line.x2, line.y2]}
                              stroke={isSelected ? "#FFD700" : "#66BBFF"}
            ...
            <div className={styles.rewardContent}>
              ...
              <div className={styles.rewardCard}>
                <div className={styles.relicDisplay}>
                  <img 
                    src="/images/mirror-reflection.png" 
                    alt="Mirror of Reflection" 
                    className={styles.relicIcon}
                  />
                </div>
                <h3 className={styles.rewardName}>The Mirror of Reflection</h3>
                <p className={styles.rewardDescription}>
                  You have uncovered the paths of power! The lines of geometry bend to your understanding.
                </p>
              </div>
