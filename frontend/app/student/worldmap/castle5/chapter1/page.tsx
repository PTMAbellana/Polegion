...
type SceneType = 'opening' | 'lesson' | 'minigame' | 'quiz1' | 'quiz2' | 'quiz3' | 'quiz4' | 'reward';

const CHAPTER_KEY = 'castle5-chapter1';
...
export default function Castle2Chapter1Page() {
    ...
    const fetchQuizScore = async () => {
      if (currentScene === 'reward' && quiz?.id) {
        try {
          ...
          console.log('[castle5-chapter1] Fetched quiz attempts:', attempts);
          ...
          if (attempts && attempts.length > 0) {
            ...
            console.log('[castle5-chapter1] Most recent quiz score:', mostRecentScore);
  ...
  React.useEffect(() => {
    // Castle 5 Chapter 1 has 6 lesson concepts with semantic keys
  ...
  function handleDialogueComplete() {
    if (currentScene === 'opening') {
      ...
      playNarration('castle5-chapter1-lesson-intro');
  ...
  const awardXP = async (type: 'lesson' | 'minigame' | 'quiz') => {
    ...
    else if (type === 'quiz') xp = CHAPTER1_XP_VALUES.quiz1 + CHAPTER1_XP_VALUES.quiz2 + CHAPTER1_XP_VALUES.quiz3 + CHAPTER1_XP_VALUES.quiz4;
  ...
  const handleQuizSubmit = async (quizNumber: 1 | 2 | 3 | 4) => {
    if (!quiz || !userProfile?.id) return;

    const taskKey = quizNumber === 1 ? 'task-7' : quizNumber === 2 ? 'task-8' : quizNumber === 3 ? 'task-9' : 'task-10';
    ...
    if (userAnswer === question.correctAnswer) {
      ...
      setTimeout(() => {
        ...
        if (quizNumber === 1) {
          ...
          // All quizzes complete (after quiz4)
          awardXP('quiz');
          
          // Submit quiz attempt
          try {
            console.log('[castle5-chapter1] Submitting quiz with answers:', quizAnswers);
            console.log('[castle5-chapter1] Quiz config:', quiz.quiz_config);
      ...
      setTimeout(() => {
        ...
        if (quizNumber === 1) {
          ...
          // All quizzes answered (even if some wrong) - after quiz4
          // Submit quiz attempt
          try {
            submitQuizAttempt(quiz.id, quizAnswers);
          } catch (error) {
  ...
  const handleRetakeQuiz = () => {
    ...
    // Clear quiz task completions (task-7 through task-10)
    setCompletedTasks((prev) => {
      const updated = { ...prev };
    ...
    <div className={`${baseStyles.chapterContainer} ${baseStyles.castle5Theme}`}>
      ...
      <div className={baseStyles.mainContent}>
        ...
        <div className={baseStyles.gameArea}>
          ...
          {currentScene === 'minigame' && (
            <ShapeBasedMinigame
              question={{
                ...
                correctAnswer: CHAPTER1_MINIGAME_LEVELS[currentMinigameLevel].type,
          ...
          {/* Quiz Scenes */}
          {(currentScene === 'quiz1' || currentScene === 'quiz2' || currentScene === 'quiz3' || currentScene === 'quiz4') && quiz && (
