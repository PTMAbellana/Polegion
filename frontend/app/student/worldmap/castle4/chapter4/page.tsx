...
const CHAPTER_KEY = 'castle4-chapter4';
...
export default function Castle2Chapter1Page() {
  ...
  const {
    ...
    dialogue: currentScene === 'opening' ? CHAPTER4_OPENING_DIALOGUE : 
             currentScene === 'lesson' ? CHAPTER4_LESSON_DIALOGUE.map(d => d.text) : 
             CHAPTER4_MINIGAME_DIALOGUE,
    ...
    const fetchQuizScore = async () => {
      if (currentScene === 'reward' && quiz?.id) {
        try {
          ...
          console.log('[castle4-chapter4] Fetched quiz attempts:', attempts);
          ...
          if (attempts && attempts.length > 0) {
            ...
            console.log('[castle4-chapter4] Most recent quiz score:', mostRecentScore);
  ...
  React.useEffect(() => {
    // Castle 4 Chapter 4 has 6 lesson concepts with semantic keys
    if (currentScene === 'lesson' && messageIndex >= 0 && messageIndex < CHAPTER4_LESSON_DIALOGUE.length) {
      const currentDialogue = CHAPTER4_LESSON_DIALOGUE[messageIndex];
  ...
  function handleDialogueComplete() {
    if (currentScene === 'opening') {
      ...
      playNarration('castle4-chapter4-lesson-intro');
  ...
  const awardXP = async (type: 'lesson' | 'minigame' | 'quiz') => {
    ...
    else if (type === 'quiz') xp = CHAPTER4_XP_VALUES.quiz1 + CHAPTER4_XP_VALUES.quiz2 + CHAPTER4_XP_VALUES.quiz3 + CHAPTER4_XP_VALUES.quiz4 + CHAPTER4_XP_VALUES.quiz5;
    ...
    if (userAnswer === question.correctAnswer) {
      ...
      setTimeout(() => {
        ...
        if (quizNumber === 1) {
          ...
          // Submit quiz attempt
          try {
            console.log('[castle4-chapter4] Submitting quiz with answers:', quizAnswers);
            console.log('[castle4-chapter4] Quiz config:', quiz.quiz_config);
    ...
    <div className={`${baseStyles.chapterContainer} ${baseStyles.castle4Theme}`}>
      ...
      {showProgressModal && (
        <ChapterProgressModal
          chapterTitle="Chapter 4: The Hall of Rays"
        ...
        <div className={baseStyles.gameArea}>
          ...
          {currentScene === 'lesson' && (
            <LessonGrid columns={3} gap="medium" styleModule={lessonStyles}>
              {CHAPTER4_CONCEPTS.map((concept, index) => {
                // Find the corresponding dialogue to determine if it's been reached
                const dialogueIndex = CHAPTER4_LESSON_DIALOGUE.findIndex(d => d.key === concept.key);
          ...
          {currentScene === 'minigame' && (
            <AreaCalculationMinigame
              question={{
                ...
                instruction: CHAPTER4_MINIGAME_LEVELS[currentMinigameLevel].instruction,
                correctAnswer: CHAPTER4_MINIGAME_LEVELS[currentMinigameLevel].correctAnswer,
                shape: CHAPTER4_MINIGAME_LEVELS[currentMinigameLevel].shape,
