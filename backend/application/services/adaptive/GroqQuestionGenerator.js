...
class GroqQuestionGenerator {
  ...
  _buildQuestionPrompt(topicName, topicFilter, difficultyLevel, cognitiveDomain) {
    ...
    return `You are an expert geometry teacher creating a ${difficultyDescription} question for Grade 6 students (ages 11-12).
...
GRADE 6 CONSTRAINTS:
- Use vocabulary appropriate for 11-12 year olds (avoid advanced terms like "subtend", "congruent polygons")
- Keep calculations within Grade 6 scope: whole numbers, simple fractions, decimals to hundredths
- Real-world contexts: classroom, playground, sports, simple everyday objects
- Maximum 2-step problems (e.g., "find length, then find perimeter" is OK; 3+ steps too complex)
- Sentence length: 15-20 words max per sentence
  ...
  _parseQuestionResponse(responseText) {
    try {
      ...
      if (correctOption.label !== question.correctAnswer) {
        console.error('[AIQuestionGenerator] CRITICAL: correctAnswer field does not match correct option!');
        console.error('Question:', question.questionText);
        console.error('Options marked correct:', correctOptions.map(o => o.text));
        console.error('correctAnswer field:', question.correctAnswer);
        // Reject this question - don't try to fix it
        return null;
      }
      
      // ADDITIONAL VALIDATION: For numeric answers, verify it's mathematically reasonable
      const correctText = correctOption.text.trim();
      const questionLower = question.questionText.toLowerCase();
      
      // Extract numbers from question to verify answer plausibility
      const questionNumbers = question.questionText.match(/\d+/g)?.map(Number) || [];
      const answerNumbers = correctText.match(/\d+/g)?.map(Number) || [];
      
      if (answerNumbers.length > 0 && questionNumbers.length > 0) {
        const answerValue = answerNumbers[0];
        const maxQuestionNum = Math.max(...questionNumbers);
        
        // Sanity check: answer shouldn't be absurdly larger than input numbers
        if (answerValue > maxQuestionNum * 1000) {
          console.error('[AIQuestionGenerator] Answer seems unreasonably large:', answerValue);
          console.error('Question numbers:', questionNumbers);
          return null;
        }
      }
