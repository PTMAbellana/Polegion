 ...
 * 2. Hybrid design: Tries OpenAI first, falls back to Groq automatically
 * 3. Rule-based fallbacks ALWAYS available if both AI providers fail
 * 4. Never blocks learning - failures are silent
 * 5. Rate limiting and caching to minimize costs
 * 
 * PRODUCTION REALITY (Jan 2026 Study):
 * - OpenAI GPT-4o-mini configured but exceeds quota (Error 429)
 * - **Groq Llama 3.3 70B is the ACTUAL provider used**
 * - All hints generated by Groq due to OpenAI quota limitations
 * - System automatically falls back with no interruption to students
 * 
 * FOR PAPER/DOCUMENTATION:
 * Report AI provider as: "GroqCloud Llama 3.3 70B"
 */
...
class HintGenerationService {
  constructor() {
    // Use hybrid AI service (OpenAI configured but quota exceeded, Groq actually used)
